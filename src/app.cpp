...
static std::vector<char> readFile(const std::string& filename) {
    ...
    std::println("Opening file: {0}", filename); 
...
void App::createGraphicsPipeline() {
    ...
    std::println("CWD: {0}", std::filesystem::current_path().string());
    // TODO well, this
...
void App::loadModel() {
    ...
    auto model = readFile(MODEL_PATH);
    auto p = model.data();

    std::println("MODEL DATA LENGTH: {0}", model.size());
    ...
    std::println("VERTICES SIZE: {0}", vertices.size());
    std::println("INDICES SIZE: {0}", indices.size());
...
void App::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSample, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
void App::createTextureSampler() {
    ...
    std::println("Current total mip levels: {0}", mipLevels);
}
...
void App::createLogicalDevice() {
    ...
    deviceFeatures.sampleRateShading = VK_TRUE; // enable sample shading feature for the device (smooth out textures) see also createGraphicsPipeline()
...
QueueFamilyIndices App::findQueueFamilies(VkPhysicalDevice device) {
    ...
    for (const auto& queueFamily : queueFamilies) {
        ...
        Nevertheless, you could add logic to explicitly prefer a physical device that supports 
